...
import (
	...
	utils "github.com/meshplus/bitxhub-core/third_party/github.com/hyperledger/fabric/protoutil"
)
...
func TemplateImplicitMetaPolicyWithSubPolicy(path []string, policyName string, subPolicyName string, rule cb.ImplicitMetaPolicy_Rule) *cb.ConfigGroup {
	root := utils.NewConfigGroup()
	group := root
	for _, element := range path {
		group.Groups[element] = utils.NewConfigGroup()
